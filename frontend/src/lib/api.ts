...
// ==========================================
// RAILPULSE-X DYNAMIC ETA API (SIH26028)
// ==========================================

export type CorridorOverviewResponse = {
	corridor_name: string;
	corridor_length_km: number;
	total_stations: number;
	active_coaching_trains: number;
	avg_corridor_delay_min: number;
	delayed_trains_count: number;
	on_time_ratio: number;
	congested_sections: Array<{ section: string; active_trains: number; congestion_level: string }>;
	system_status: string;
	model_version: string;
	updated_at: string;
};

export type LiveTrainEta = {
	train_no: string;
	train_name: string;
	train_type: string;
	priority: number;
	...
	speed_kmph: number;
	delay_minutes: number;
	preceding_train_no?: string | null;
	preceding_distance_km?: number | null;
	preceding_delay_min?: number;
	telemetry_age_sec: number;
	status: string;
};

export type StationEtaPrediction = {
	station_code: string;
	station_name: string;
	distance_km: number;
	is_junction: boolean;
	...
	scheduled_arrival: string;
	scheduled_departure: string;
	baseline_official_eta: string;
	baseline_delay_min: number;
	railpulse_eta_p10: string;
	railpulse_eta_p50: string;
	railpulse_eta_p90: string;
	dynamic_delay_p50_min: number;
	uncertainty_band_min: number;
	delta_vs_baseline_min: number;
	confidence_score: number;
	recovery_allowance_min: number;
	delay_drivers: {
		baseline_carryover: number;
		preceding_train_impact: number;
		dwell_variance: number;
		schedule_recovery_credit: number;
		staleness_drift_penalty: number;
	};
};

export type EtaPredictionResponse = {
	train_no: string;
	train_name: string;
	train_type: string;
	current_station: string;
	...
	predictions: StationEtaPrediction[];
...
export type BacktestBenchmarkResponse = {
	summary: {
		...
		test_sample_runs: number;
		total_arrival_checkpoints: number;
		baseline_model_name: string;
		railpulse_model_name: string;
		baseline_mae_min: number;
		railpulse_mae_min: number;
		mae_reduction_minutes: number;
		mae_improvement_percent: number;
		baseline_rmse_min: number;
		railpulse_rmse_min: number;
		rmse_improvement_percent: number;
		baseline_median_error_min: number;
		railpulse_median_error_min: number;
		quantile_nominal_band: string;
		quantile_empirical_coverage_pct: number;
	...
	horizon_analysis: Array<{
		horizon_label: string;
		lookahead_minutes: number;
		baseline_mae: number;
		railpulse_mae: number;
		improvement_pct: number;
		sample_count: number;
	}>;
	station_benchmarks: Array<{
		station_code: string;
		station_name: string;
		baseline_mae: number;
		railpulse_mae: number;
	...
	sample_journeys: Array<{
		...
		train_name: string;
		test_date: string;
		checkpoint_station: string;
		actual_arrival_time: string;
		baseline_predicted_time: string;
		baseline_error_min: number;
		railpulse_predicted_p50: string;
		railpulse_predicted_band: string;
		railpulse_error_min: number;
		inside_band: boolean;
		why_baseline_failed: string;
	}>;
	verified_at: string;
};
...
export type WhatIfResponse = {
	scenario: {
		...
		injected_extra_delay_min: number;
	};
	impact_summary: {
		destination_delay_increase_min: number;
		cascading_absorption_ratio: number;
		insight: string;
	};
	original_forecast: StationEtaPrediction[];
	simulated_forecast: StationEtaPrediction[];
};

import {
	fallbackCorridorOverview,
	fallbackLiveTrains,
	fallbackBenchmarkData,
	generateClientEtaPrediction,
	generateClientWhatIf
} from './etaFallback';

export async function fetchEtaOverview(): Promise<CorridorOverviewResponse> {
	try {
		const res = await fetch(`${apiBaseUrl}/api/eta/overview`);
		if (res.ok) return (await res.json()) as CorridorOverviewResponse;
	} catch (e) {
		// graceful fallback for static/Vercel deployment
	}
	return fallbackCorridorOverview;
}

export async function fetchEtaLiveTrains(): Promise<LiveTrainEta[]> {
	try {
		const res = await fetch(`${apiBaseUrl}/api/eta/live-trains`);
		if (res.ok) return (await res.json()) as LiveTrainEta[];
	} catch (e) {
		// graceful fallback for static/Vercel deployment
	}
	return fallbackLiveTrains;
}

export async function fetchEtaPrediction(trainNo: string, stalenessSec?: number): Promise<EtaPredictionResponse> {
	try {
		const url = stalenessSec !== undefined
			? `${apiBaseUrl}/api/eta/predict/${trainNo}?staleness_sec=${stalenessSec}`
			: `${apiBaseUrl}/api/eta/predict/${trainNo}`;
		const res = await fetch(url);
		if (res.ok) return (await res.json()) as EtaPredictionResponse;
	} catch (e) {
		// graceful fallback for static/Vercel deployment
	}
	return generateClientEtaPrediction(trainNo, stalenessSec);
}

export async function fetchBacktestBenchmark(): Promise<BacktestBenchmarkResponse> {
	try {
		const res = await fetch(`${apiBaseUrl}/api/eta/benchmark`);
		if (res.ok) return (await res.json()) as BacktestBenchmarkResponse;
	} catch (e) {
		// graceful fallback for static/Vercel deployment
	}
...
export async function simulateWhatIfDisruption(payload: {
	...
	try {
		const res = await fetch(`${apiBaseUrl}/api/eta/what-if`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload),
		});
		if (res.ok) return (await res.json()) as WhatIfResponse;
	} catch (e) {
		// graceful fallback for static/Vercel deployment
	}
	return generateClientWhatIf(payload);
}

export async function updateTrainTelemetry(payload: {
	train_no: string;
	current_station?: string;
	next_station?: string;
	current_km?: number;
	...
	const res = await fetch(`${apiBaseUrl}/api/eta/telemetry-update`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify(payload),
	});
	if (!res.ok) throw new Error('Failed to update train telemetry');
	return await res.json();
}
