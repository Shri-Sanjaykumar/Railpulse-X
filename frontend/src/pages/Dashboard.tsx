...
export default function DashboardPage() {
  ...
  return (
		<div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-indigo-50 p-4 sm:p-6 lg:p-8">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 flex items-center gap-2">
							RailPulse-X Operations
							<span className="text-xs px-2.5 py-0.5 rounded-full bg-blue-600 text-white font-semibold">SIH26028</span>
						</h1>
						<p className="text-xs text-gray-600 font-medium">Real-Time Delay Monitoring & Dynamic Arrival Forecasting (KTV-PSA Corridor)</p>
					</div>
        </div>
			...
			{/* RailPulse-X Dynamic ETA Feature Hero Banner */}
			<div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 border border-blue-700/50">
				<div className="space-y-1">
					<div className="flex items-center gap-2">
						<span className="px-2 py-0.5 rounded text-[10px] font-bold bg-cyan-400/20 text-cyan-300 border border-cyan-400/30 uppercase">
							Dynamic ETA Engine Active
						</span>
						<span className="text-xs text-emerald-400 font-medium">
							Beats Schedule+Delay Baseline by 66.0%
						</span>
					</div>
					<h3 className="font-bold text-lg text-white">Dynamic Quantile ETA & Held-Out Backtest Verification</h3>
					<p className="text-xs text-slate-300 max-w-2xl">
						Official NTES static points ignore sectional ripple and timetable slack. Switch to the new RailPulse-X Dynamic Arrival View to inspect [P10-P90] uncertainty bands.
					</p>
				</div>
				<div className="flex items-center gap-2 shrink-0">
					<button
						onClick={() => navigate('/app/eta')}
						className="px-4 py-2 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs transition shadow-lg flex items-center gap-1.5"
					>
						<Clock size={14} /> Open Dynamic ETA Studio
					</button>
					<button
						onClick={() => navigate('/app/benchmark')}
						className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-semibold text-xs border border-slate-600 transition"
					>
						Backtest Benchmark
					</button>
				</div>
